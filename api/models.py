...
class SymbolMetrics(BaseModel):
    ...
    long_extension_score: Optional[float] = Field(None, ge=0, le=1, description="Long-side extension score: penalises entries near the top of the 15m range.")
    short_extension_score: Optional[float] = Field(None, ge=0, le=1, description="Short-side extension score: penalises entries near the bottom of the 15m range.")
    long_exhaustion_risk: Optional[float] = Field(None, ge=0, le=1, description="Long-side exhaustion risk derived from stretch, range edge, RSI, volume, and local trend quality.")
    short_exhaustion_risk: Optional[float] = Field(None, ge=0, le=1, description="Short-side exhaustion risk derived from stretch, range edge, RSI, volume, and local trend quality.")
    ...
    long_entry_risk: Optional[float] = Field(None, ge=0, le=1, description="Long entry-location risk: high when near range top, resistance, or overbought 5m RSI. Caps long_score when >= 0.70.")
    short_entry_risk: Optional[float] = Field(None, ge=0, le=1, description="Short entry-location risk: high when near range bottom, support, or oversold 5m RSI. Caps short_score when >= 0.70.")
    range_position_15m: Optional[float] = Field(None, ge=0, le=1, description="Price position within the 20-bar 15m high/low range [0=low, 1=high].")
    range_position_5m_12: Optional[float] = Field(None, ge=0, le=1, description="Price position within the 12-bar 5m range [0=low, 1=high]. Matches CoinR analysis entry-range logic.")
    support_distance_pct_15m: Optional[float] = Field(None, ge=0, description="Percentage distance from current price down to nearest support (15m near + 1h key, most restrictive).")
    resistance_distance_pct_15m: Optional[float] = Field(None, ge=0, description="Percentage distance from current price up to nearest resistance (15m near + 1h key, most restrictive).")
    support_touches_15m: Optional[int] = Field(None, ge=0, description="Number of 15m bars (last 8) that touched the near support level within adaptive ATR tolerance.")
    resistance_touches_15m: Optional[int] = Field(None, ge=0, description="Number of 15m bars (last 8) that touched the near resistance level within adaptive ATR tolerance.")
    rsi_5m: Optional[float] = Field(None, ge=0, le=100, description="RSI (14) on the 5m timeframe. Used for rsi_chase_long (>78) / rsi_chase_short (<22) caps.")
    taker_dominance_5m: Optional[TakerDominance] = Field(None, description="Taker buy/sell dominance over the last 10 x 5m bars (ratio > 1.15 / < 0.85). Matches CoinR analysis taker check.")
...
class TradeSetup(BaseModel):
    ...
    signal_id: str = Field("", description="Stable id used to trace analysis -> signal -> trade flow.")
    size_multiplier: float = Field(1.0, description="Deterministic position-size multiplier chosen by CoinR.")
    sizing_reasons: List[str] = Field(default_factory=list, description="Reasons that produced the final deterministic size multiplier.")
    notes: List[str] = Field(default_factory=list, description="Analyst notes generated by the LLM.")
    decision_context: Dict[str, Any] = Field(default_factory=dict, description="Deterministic routing and embedded market snapshot metadata from CoinR.")
    review: Dict[str, Any] = Field(default_factory=dict, description="Reviewer LLM output and fallback review metadata from CoinR.")
